...
import { LayoutDashboard, Wallet, DollarSign, TrendingUp, TrendingDown, FileText, LogOut, Menu, X, Bell } from "lucide-react";
...
const NAV = [
  ...
  { to: "/finance/fees", label: "Student Fees", icon: DollarSign },
  { to: "/finance/structure", label: "Fee Structure", icon: Wallet },
...
export function FinanceLayout({ children, title }: { children: React.ReactNode; title?: string }) {
      ...
      <div className="min-h-screen flex bg-background">
        <aside className={cn("fixed inset-y-0 left-0 z-40 w-64 border-r bg-sidebar text-sidebar-foreground transition-transform lg:static lg:translate-x-0", open ? "translate-x-0" : "-translate-x-full")}>
          <div className="flex h-16 items-center gap-2 border-b px-4">
            <div className="grid h-9 w-9 place-items-center rounded-lg bg-[var(--gold)]"><Wallet className="h-5 w-5" /></div>
            <div><div className="font-display text-sm font-bold">Blessing School</div><div className="text-[10px] uppercase tracking-widest opacity-70">Finance</div></div>
        ...
        <div className="flex-1 flex flex-col">
          <header className="flex h-16 items-center gap-3 border-b px-4">
            ...
            <div className="ml-auto flex items-center gap-1">
              ...
              <ThemeToggle />
              <div className="mr-2 hidden sm:flex flex-col items-end leading-tight"><div className="text-sm font-medium">{user.email}</div><div className="text-xs text-muted-foreground">Finance</div></div>
