...
import { LayoutDashboard, Package, TrendingUp, TrendingDown, FileText, Bell as BellIcon, LogOut, Menu, X, Bell } from "lucide-react";
...
const NAV = [
  { to: "/stock", label: "Dashboard", icon: LayoutDashboard, exact: true },
  { to: "/stock/products", label: "Products", icon: Package },
  { to: "/stock/in", label: "Stock In", icon: TrendingUp },
  { to: "/stock/out", label: "Stock Out", icon: TrendingDown },
  { to: "/stock/reports", label: "Reports", icon: FileText },
  { to: "/stock/notifications", label: "Alerts", icon: BellIcon },
];

export function StockLayout({ children, title }: { children: React.ReactNode; title?: string }) {
      ...
      <div className="min-h-screen flex bg-background">
        <aside className={cn("fixed inset-y-0 left-0 z-40 w-64 border-r bg-sidebar text-sidebar-foreground transition-transform lg:static lg:translate-x-0", open ? "translate-x-0" : "-translate-x-full")}>
          <div className="flex h-16 items-center gap-2 border-b px-4">
            <div className="grid h-9 w-9 place-items-center rounded-lg bg-[var(--gold)]"><Package className="h-5 w-5" /></div>
            <div><div className="font-display text-sm font-bold">Blessing School</div><div className="text-[10px] uppercase tracking-widest opacity-70">Stock</div></div>
        ...
        <div className="flex-1 flex flex-col">
          <header className="flex h-16 items-center gap-3 border-b px-4">
            ...
            <div className="ml-auto flex items-center gap-1">
              ...
              <ThemeToggle />
              <div className="mr-2 hidden sm:flex flex-col items-end leading-tight"><div className="text-sm font-medium">{user.email}</div><div className="text-xs text-muted-foreground">Stock</div></div>
