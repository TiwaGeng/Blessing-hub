...
import { canAccessRoute, getDashboardRoute } from "@/lib/role-access";
...
export function AdminLayout({
  ...
  const { user, signOut, loading, roles } = useAuth();
  const navigate = useNavigate();
  const path = useRouterState({ select: (s) => s.location.pathname });

  // Check if user has access to current route
  if (!loading && user && !canAccessRoute(roles, path)) {
    const dashboardRoute = getDashboardRoute(roles);
    if (typeof window !== "undefined") navigate({ to: dashboardRoute });
    return (
