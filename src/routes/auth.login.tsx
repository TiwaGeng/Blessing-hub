...
function Login() {
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (rolesError) {
        toast.error(
          "Signed in, but could not load your role. Please contact the admin.",
        );
        return;
      }
      ...
      if (roles.length === 0) {
        toast.error("Signed in, but no role is assigned to this account.");
        return;
      }
    ...
    <div className="min-h-screen grid lg:grid-cols-2 bg-background text-foreground font-sans">
      <div className="hidden lg:flex relative bg-primary text-primary-foreground p-12 flex-col justify-between overflow-hidden">
        <div
          ...
          style={{
            backgroundImage:
              "radial-gradient(circle at 20% 20%, var(--gold) 0, transparent 35%), radial-gradient(circle at 80% 80%, #2563eb 0, transparent 35%)",
          }}
        />
        <Link to="/" className="relative flex items-center gap-2">
          <div className="h-10 w-10 rounded-lg bg-[var(--gold)] text-[var(--gold-foreground)] grid place-items-center">
        ...
        <div className="relative">
          ...
          <p className="mt-4 text-white/70 max-w-md">
            Admins, teachers, finance, library and stock teams — manage your
            daily workflow from one connected dashboard.
          </p>
        </div>
        <div className="relative text-xs text-white/60">
          © {new Date().getFullYear()} Blessing School
        </div>
      </div>
      ...
      <div className="flex items-center justify-center p-6 sm:p-12">
        <div className="w-full max-w-md">
          ...
          <p className="text-sm text-muted-foreground mt-1">
            Use your school account to access your dashboard.
          </p>
          ...
          <Card className="mt-6">
            <CardContent className="p-6">
              ...
              <div className="mt-6 rounded-lg border border-dashed border-border p-3 text-xs text-muted-foreground">
                ...
                <div>Finance — finance@blessingschool.com / Finance@12345</div>
                <div>Library — library@blessingschool.com / Library@12345</div>
                <div>Stock — stock@blessingschool.com / Stock@12345</div>
              </div>
              <div className="mt-4 text-xs text-center">
                ...
                >
                  Parent? Use the Parent Portal instead →
                </Link>
              </div>
            </CardContent>
          </Card>
